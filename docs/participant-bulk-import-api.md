...
## 변경 이력 (2026-07-29)
- **미리보기 행(`BulkImportParsedRow`)에 `selected`(선정여부 원문: 선정/미선정/공란) 노출.**
  `ParticipantExcelParser`가 `선정여부` 값을 그대로 전달(기존엔 등록 상태 계산에만 사용). `선정일시`는 기존 `receptionDate`.
  FE가 "선정 인원만" 기본 선택·확인 후 등록하는 데 사용. commit 계약은 불변(개별 item 배열 그대로 수신).
